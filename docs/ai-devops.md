...
- **release/vX.Y.Z = 次版の統合ブランチ**（main と同じ保護: test 必須 + 未解決スレッドでマージ不可）。
  開発 PR はすべてここにスタックする。
- **Issue は起票時にマイルストーン（対象バージョン）を必ず設定する**。
...
- **`web/`（LP）はリリース列車に乗せない**（2026-08-20 追加・#176）。LP は Apple の審査を経ず、本番デプロイ（Vercel）は
  **main 追従**であるため、release ブランチに積むと「そのバージョンが App Store で公開されて main にマージされるまで
  LP も塩漬け」になる。#156（LP を実装の10本に追従）が `release/v1.1.2` 止まりだったために、大富豪・麻雀ソリティアの
  ページが本番で3日間 404 のままだった。`web/` の変更は上の運用系と同じく **main 直の PR** とし、テスト・ビルドが
  緑になった時点でデプロイする。アプリコードと `web/` を同時に触る Issue は、`web/` だけを main 直 PR に分離する。
...
- アプリコードの PR のベースは **Issue のマイルストーンと同名の release ブランチ**（現在動いているのは
  `release/v1.1.1` のみ。`release/v1.1.0` は審査提出済みのため凍結中で push 禁止）。
  運用系のみの変更は main 直可。
- **2026-08-24: v1.1.1 と v1.1.2 を統合**（会長判断）。v1.1.0 の審査が長引いており、レビューサイクル自体が
  ボトルネックなため、2回に分けてリリースするより1回にまとめる方が効率的と判断。`release/v1.1.2`
  （UI/UXの磨き込み含む全内容。この時点で既に `release/v1.1.1` を fast-forward で含んでいた）を
  `release/v1.1.1` へ fast-forward し、`release/v1.1.2` ブランチとマイルストーンは退役させた
  （`project.yml` の `MARKETING_VERSION` は元々 `release/v1.1.2` 上でも `"1.1.1"` のままだったため
  変更不要だった。バージョン番号の更新は配信直前の手作業であり、ブランチ作成時点では一致しないのが
  正常な設計のため）。両マイルストーンに残っていたオープン Issue は全て `v1.1.1` へ付け替え済み。
  次の新規バージョンには `v1.1.3` マイルストーン（作成済み）を使う。

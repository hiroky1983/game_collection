...
/// A′（2D）と C（SceneKit）が**同じ断面**を使うので、比較は「描き方の差」だけになる。
typealias Prof = [(r: CGFloat, y: CGFloat)]
...
enum RealProfiles {
    ...
    static func smooth(_ p: Prof, iterations: Int = 3) -> Prof {
        var cur = p

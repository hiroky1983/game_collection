...
1. **タップ標的 44pt**（`Metrics.toggleButtonMinSide`）。値を Metrics に集約したうえで、
   **View がその定数を `.frame` に渡していること自体をソース走査で固定した**
   （`MotionTests.noRawAnimationOutsideCore` と同じやり方）。定数だけ突き合わせても、
   View 側を `minWidth: 30` のようにハードコードで書き換える改変は素通りしてしまうため。
   実際に `minWidth: 30` / `.padding(.vertical, 8)` へ書き換えて**テストが赤くなることを確認済み**。
...
5. **ボタンスタイルは `.pop`**（`.plain` ではない）。`.plain` は自前で描いた背景を通す代わりに
   押下フィードバックまで消すため、まさにこの用途で用意されている `PopButtonStyle`（#195）を使う。
   休止状態の見た目は `.plain` と変わらない（押下中だけ 0.96 倍に縮んで薄くなる）。

...
### 最後の1組（CodeRabbit の指摘・PR #235）

![最後の1組](05-last-pair.jpg)

`tap(_:)` は最後の 1 組の `faces` を nil にしたのと**同じ更新**で `phase` を `.won` にするため、
盤面が `model.phase` を直に見ていると盤面ごとクリア表示に差し替わり、**最後の 2 枚だけ演出が飛ぶ**。
盤面の表示だけを `showsClearDisplay`（`Metrics.boardAnimationDuration` だけ遅らせて立てる）で切り替え、
最後の 1 組も同じ演出で消えるようにした。**勝敗・記録・計時（`model` 側）は従来どおり即座に確定する**
（遅らせたのは盤面の見た目だけ）。

上の帯は `model.solution` の順に自動で取り切るプローブビルド（同じく 4.0s に伸ばして録画）で撮ったもので、
七萬の最後の 1 組が縮みながら消え、そのあとにクリア表示が出ている。Reduce Motion が ON のときは
待ち時間そのものを飛ばす（演出が無いので待つ意味が無い）。
...
## `release/v1.1.3`（#198 のアンドゥ）との統合

![統合後](06-merged-with-198.jpg)

`release/v1.1.3` では #198 が操作バーを **3 ボタン（ヒント・並べ替え・戻す）+ `ViewThatFits`** に
組み替えており、`gameControls` がまるごと衝突した。#198 は**アイコンだけの段にしか 44pt を入れず**、
文字付きの段は「ヒント・並べ替えと共通の見直しになるため #199 のスコープ」と保留していた
（`controlButton` のコメント）。そのため統合は次のとおり:

- **構造は #198 側（3 ボタン + `ViewThatFits`）を採用**。#199 側の 2 ボタン構成は捨てる。
- **高さの下限を `showsTitle ? nil : …` から `Metrics.controlButtonMinHeight` に変え、両方の段・3 つの
  ボタンすべてで 44pt にした**（#198 のコメントが言う「1 つだけ大きくすると帯が不揃いになる」の解消）。
- #199 側で入れた `.lineLimit(1)` と回数表示の縮小指定は不要になった。#198 が回数表示そのものを
  やめている（回数はリザルトに出る）ため、折り返しの原因が無くなっている。

統合後の実測（iPhone 17 Pro）: ヒント **82.0 × 44.0pt** / 並べ替え **101.3 × 44.0pt** /
戻す **71.0 × 44.0pt**。3 つとも 1 段に収まり、文字の折り返しも無い。

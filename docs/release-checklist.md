...
- **v1.1.7 の審査提出で実績 4 件を同梱する**（#1153・2026-09-25 社長が ASC に登録済み）。`firstwin`（25点）・`wins10`（75点）・
  `wins50`（100点）・`playall`（50点）を作成し、「提出物の下書き」に 4 件とも追加済み。**アプリで最初の実績は
  Game Center 対応のアプリバージョンと一緒にしか提出できない**（単独提出は ASC が拒否）ため、v1.1.7 の
  審査提出の画面でこの下書きも一緒に「審査へ提出」する。画像は仮（丸にタイトル文字）で、後から差し替え可
...
- [ ] **実機テスト**（シミュレーターでは確認できない項目）
  ...
  - ATT ダイアログ表示確認
  - 各ゲームの動作確認
  - ハブのゲームカード（グリッドと「つづき・最近」の行）を押している間だけ沈み、離すと戻る。Reduce Motion ON でも縮小は残る（v1.1.5〜・PR #774。シミュレータでは押下状態を自動で作れず、`docs/ui-review/823/774-hub-card-pressed.jpg` は押下を強制した一時ビルドの画）
  - ブロック崩しのパドルが盤の上でも盤の下の余白でも指の移動量で動き、指でパドルが隠れない（v1.1.5〜・PR #803。倍率 1.5 の感触は実機でしか確かめられない）

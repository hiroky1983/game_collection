...
- [ ] **実機テスト**（シミュレーターでは確認できない項目）
  ...
  - ATT ダイアログ表示確認
  - 各ゲームの動作確認
  - ハブのゲームカード（グリッドと「つづき・最近」の行）を押している間だけ沈み、離すと戻る。Reduce Motion ON でも縮小は残る（v1.1.5〜・PR #774。シミュレータでは押下状態を自動で作れず、押下を強制した一時ビルドの画は #1409 で `docs/ui-review/` ごと削除したので、必要なら git 履歴から取り出す）

...
- 解析（`AnalyticsLevel`）は既存 3 段階の値（`beginner`/`normal`/`hard`）を動かさず、両端に
  `novice`（入門）・`expert`（ガチ）を追加した5値。GA4 に貯まっている既存3段階の数字はそのまま続けて読める
...
### バカラ（#1197・`release/v1.1.6`）

プレイヤー／バンカー／タイの3択に賭ける公式ルールのバカラ。CPU 思考が無いため `GameBaccarat` は
`CoreEngine` に依存しない（ルールは `BaccaratRules.swift` に純関数として切り出し）。

- 手の合計は下1桁のみ（7+8=15→5）。**A は 1 点固定**（ブラックジャックと違い 11 にはならない）、10・絵札は 0 点
- 最初の2枚で8か9（ナチュラル）が出ればどちらも3枚目を引かず即決着。3枚目の引き足しは公式表どおり
  （プレイヤーは2枚合計0〜5で引く。バンカーはプレイヤーの3枚目の点数を見て0〜7の範囲で公式表に従う）
- 配当: プレイヤー勝ち **1倍**・バンカー勝ち **0.95倍**（手数料5%。ハウスエッジの原資）・タイ的中 **8倍**。
  タイが出たときプレイヤー／バンカーへの賭けは**引き分け**（賭け金をそのまま返す。没収しない）
- チップ経済はブラックジャック・ポーカー（上記 #499/#523）に揃える: 初期 **1000**・最小ベット **50**・
  リワード広告での復活は **2000**（無料のやり直しより必ず多い）・**1セッション1回**まで。
  復活したセッションは Game Center の順位表（`asobiba.baccarat.chips`）から除外（`isLeaderboardEligible`）
  するが、端末内の自己ベストには残す。復活の提示は `reward_offer` に結線済み
- 中断データは**賭け待ちの状態（復活後のチップだけ）**しか持たない（#1104 と同型）。1回の賭けは
  賭けた瞬間に決着まで進むため「途中の局」が存在せず、`GameModule.hasResumableSnapshot` は常に `false`
  （ハブの「続きから」・`game_open` の `resume` には数えない）

### 七並べ（#1198・`release/v1.1.6`）

人間1人 + CPU3人（`SevensRules.playerCount`）の標準ルール七並べ。CPU 思考は探索を持たない貪欲法のため
`GameSevens` も `CoreEngine` に依存しない（ルールは `SevensRules.swift` に純関数として切り出し）。

- 場は4スートぶんの「下限・上限（`SevensSuitRange`）」だけで再現する（七並べは常に連続した範囲でしか
  開通しないため、置かれた札の集合を持つ必要がない）。7 が出るまで両端は `nil`、出た後は `low - 1` か
  `high + 1` だけが出せる
- 出せる札が無ければパス（`pass()`）。出せる手があるのにパスはできない
- 最初の手番は**♦7を持つプレイヤー**（標準ルール）。手札は 52 枚 ÷ 4人 = 13枚ずつ
- CPU の選択は貪欲法: 出せる手の中から**7から最も距離が遠いランク**（A・K に近いほど将来出せる機会が
  少ないため優先して処理する）を出す。同着はカード ID の昇順で機械的に決める
- 誰かの手札が最初に0枚になったら終了（あがり）。難易度・CPU の強さ設定は持たない（`AnalyticsLevel` の
  `level` は送らない）
...
よく遊んでいたのに最近開いていないゲームへ、7日・30日・60日後の3回だけ「「将棋」、久しぶりに遊んでみませんか？」を
...
- **規則は Core の `ReengagementReminderService` / `ReengagementReminderPolicy`**、OS へ渡す部分は App の
  `UserNotificationReengagementScheduler`（`UNUserNotificationCenter`）。予約済みの状態は OS へ都度問い合わせ、アプリ側に保存先を増やさない
- **対象判定**: アプリがバックグラウンドに入るたびに判定し直す。**最終プレイから7日以上経っている**ゲームのうち、
  **通算プレイ回数（`PlayLog.totalPlaysByGame`）が最も多いもの1件**。同率はハブの並び順（先頭）で決める
- **発火**: 最終プレイ日時の7日後・30日後・60日後の19時（過去に落ちるものは予約しない）。対象が別のゲームに
  変わったら古い対象を取り消して置き換え、対象が無くなれば取り消すだけにする
- **取り消し**: そのゲームを開いた（`gameDidOpen`）・設定でオフにした。**1回でも開いたら以降は停止**（会長決裁 2026-09-20）
...
- `level`（`AnalyticsLevel`）はゲームごとの難易度呼称をゲーム横断で読める5段階
  ...
  写像は各ゲームの `analyticsLevel` に置く。`novice`/`expert` は CPU 対戦の5段階化（`CPUStrength`・#1174・
  v1.1.6。上記「CPU 対戦の強さ 5 段階」）で両端に追加した値で、既存3段階（`beginner`/`normal`/`hard`）の
  値・GA4 の既存の数字は動かしていない

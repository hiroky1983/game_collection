...
- **将棋は #1397（v1.1.7）で段階の作りを変えた**（上の 2 項は v1.1.6 時点の記述）。段階の強さは
  時間ではなく**読む局面数の上限**で決め（`ShogiEngine.swift` の `normalNodeLimit` / `hardNodeLimit`。
  時間は安全用に長めに残す。遅い端末で段階の差が消えないため）、入門・簡単・ふつうは一定の確率で
  「見逃し」（損の幅つきの乱択・`ShogiMovePolicy`）を起こす。むずかしいは当面 100% 最善手。入門は
  自分の手 1 手だけを読む（取り返される取りを指すことがある）。**上の段階は下の段階に負けない**
  （会長決裁 2026-09-25）ことを `CPUBenchTests`（`CPU_BENCH=1` のときだけ走る計測）で確かめる
...
- 広告の内容レーティング上限は `initializeAds()` が `start()` の前に `maxAdContentRating = .general`（G）へ固定する（会長決裁 2026-09-25・#1387）
...
- ヒントは盤下の操作行（`Core/BoardGameControlBar.swift`）の右端「⋯」メニューに入る（#1421。3本とも同じ部品。
  電球 + 残り回数）。パズル系（ナンプレ #262・麻雀ソリティア #336）の
...
- `hints_used`（#1326）は**そのプレイで使った無料ヒントの回数**（1〜3）。将棋・チェス・五目並べの3本だけが送る。
  ...
  1手も指さずに捨てた局は従来どおり `game_end` 自体が出ない。休憩（再開できる離脱）をまたいでも数えは残り、
  アプリを終了したあとに「続きから」で復元した局も、休憩に入る時点の控えから取り戻す（下記 #1374。`cause` だけは控えに入れない）。
...
- **休憩中にアプリが終了しても、「続きから」で戻って決着した局の `game_end` は出る**（#1374）。`GameAnalytics` が休憩に入る
  ...
  （ゲーム ID → 控えの辞書。端末内のみ・上限は登録ゲーム数・サーバ不要）。決着（`sendEnd`）・離脱・新規開始・解析設定の切替
  （`discardPlayState`）で消え、空になれば鍵ごと消える。**設定の「プレイ記録を消去」では消えない**（解析の計測状態で、プレイ記録ではない。
  残っても次の新規開始か決着で消える）。休憩せずに遊んでいる最中にアプリが終了した局は対象外

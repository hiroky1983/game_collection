...
### チャリンコおじさん（#796 #797 #798 #800 #801 #1009）
...
世界で変わるのは背景・地面・岩・動く障害の色だけで、当たり判定・速さには触れない。面の数も世界の数も `RunnerStage.all` と
`RunnerWorld.allCases` から数え、ワールドマップ・読み上げ・開始シートの説明はそこから作る。

- 19〜30 面は `release/v1.1.6` で足した（#1009。1〜18 面の並びは変えていない）。速さは `speedStep` 0.8 のまま 30 面で 57.2
- 里山・港町の配色は**仮置き**（朝の下町と同じ「淡い背景に暗い手前」）で、遠景の飾りはまだ描いていない。着せ替え（用水路・
  切り株・カラス／岸壁の切れ目・ドラム缶・カモメ…）と絵は #1009 の絵の PR で入る
- 新しい仕組み（#1010 突き上げ・#1089 沈む床・#1090 崩れる足場・#1091 高い塀）はまだ無く、置く予定の区画を今ある障害で
  仮に埋めてある（突き上げ・高い塀 → 高い岩、沈む床 → 2 タイルの穴、崩れる足場 → 3 タイルの穴）。予定の区画番号は
  `RunnerStage.patterns` の各行のコメントにあり、仕組み側の Issue がそこを自分の記号に置き換える

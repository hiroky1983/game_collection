...
### 戦略ゲームのヒント（`BoardHintModel`・#1118・v1.1.6）

将棋・チェス・五目並べに**1 局 3 回**のヒントがある（会長決裁 2026-09-19・選択肢 A）。
共通部品は `Core/BoardGameChrome.swift`（`BoardHintRules` / `BoardHintModel` / `BoardHintButton`）。

- **広告では補充しない。** 収益化の凍結が禁じているのは「収益を増やす目的で広告の面・頻度を
  増やすこと」で、#480（ソリティアの「戻す」）の例外は**詰み（それ以上進めない）の救済**に
  対して認められたもの。「負けそう」は詰みではないため例外の射程に入らない、というのが決裁の理由。
  したがってヒントは `RewardedRescue` を通らず、`reward_*` の解析イベントも出ない。
- **回数は局ごとにリセットする**（`newGame` で 0 に戻す）。使った回数は中断データに
  `hintsUsed` を **optional で**書き、再開後も残り回数を引き継ぐ。
- **示すのは CPU の着手と同じエンジン・同じ強さの最善手 1 手**で、盤に黄色の枠（五目並べは輪）で出す。
  指す・待った・新規対局で印は消えるが、**使った回数は戻らない**。
  回数を減らすのは手が見つかったときだけで、探索中に局面が変わったら何も起きなかったことにする（#729 と同じ考え方）。
- **CPU の手番・CPU 思考中は押せない。** その局面の最善手は CPU がこれから指す手そのもので、
  見せるとヒントではなく手の内の公開になる。
- **ヒントを使った局は `GameScore.isLeaderboardEligible` を false にする。**
  ただし 3 ゲームとも指標は `.winLoss` で、`GameCenterLeaderboard.score` が `.winLoss` を
  無条件に弾くため**そもそも順位表へは送っていない**。この旗は「将来 `.winLoss` に順位表を
  足したときに勝手に混ざらない」ための保証である。ローカルの自己ベストは使用の有無を問わず残す
  （`variant` は nil のまま。分けると記録の行が 2 つに割れる・#406）。

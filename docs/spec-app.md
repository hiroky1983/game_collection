...
Packages/GameKit/
  Sources/
    ...
    GameRoulette/       ← ルーレット（企画倉庫・ハブ未登録。2026-09-24 時点・#1318。ヨーロピアン配列 0〜36・一人でハウス相手に仮想チップを賭ける。数字 1 点 35 倍・12 個の区分 2 倍・赤黒/奇数偶数/1〜18/19〜36 は 1 倍。経済はブラックジャックと同じ（1000 枚・最小賭け 10・広告で 1 セッション 1 回 2000 枚で復活）。`AppGameServices.registry` の登録行はコメントアウトで、出荷の版が決まったら外す）
    GameFruits/         ← くっつきフルーツ（企画倉庫・ハブ未登録。2026-09-24 時点・#1319。同じ果物どうしが触れると 1 つ大きい果物になる落とし物パズル。11 種（ブルーベリー → メロン）・落とすのは小さい 5 種・得点は三角数・メロン 2 個で消えて 100 点。危険線より上に 1 秒とどまると終局で、広告コンティニュー 1 回（小さい 4 種と線より上の果物を片づける）。落下・衝突・合体は SpriteKit 非依存の純粋ロジック（`FruitField`）で、SpriteKit は描画だけ。名称・絵柄はオリジナル。`AppGameServices.registry` の登録行はコメントアウトで、出荷の版が決まったら外す）
    GameColorRelay/     ← いろリレー（企画倉庫・ハブ未登録。2026-09-24 時点・#1320。色合わせのカードゲーム（CPU 3 人と対戦）。108 枚＝4 色 × 25 枚（0 が 1 枚・1〜9 が 2 枚ずつ・とばし / ぎゃく / +2 が 2 枚ずつ）+ いろがえ 4 + いろがえ+4 の 4。場の札と同じ色か同じ数字・記号の札を出し、出せなければ山から 1 枚（出せればその札だけ出せる）。+2 / いろがえ+4 は積み重ね無しで、受けた自分の番に広告で 1 ゲーム 1 回だけ免除できる（`purpose: .revival`・手番の通し番号で局ガード）。記録は勝敗のみ（順位表無し）。CPU は難易度を持たない貪欲法。名称・札の意匠はオリジナル（白い角丸の窓 + SF Symbols + 2×2 の 4 色印）。`AppGameServices.registry` の登録行はコメントアウトで、出荷の版が決まったら外す）

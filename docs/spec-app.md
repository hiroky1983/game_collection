...
- 新しい仕組み 4 つのうち**突き上げ（#1010）・沈む床（#1089）・崩れる足場（#1090）は入った**。残る #1091 高い塀はまだ無く、
  置く予定の区画を高い岩で仮に埋めてある。予定の区画番号は `RunnerStage.patterns` の各行のコメントにあり、
  仕組み側の Issue がそこを自分の記号に置き換える

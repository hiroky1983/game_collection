...
struct ThemeContrastTests {
    ...
    @Test func inkIsReadableInBothModes() {
        for (bg, ink) in [
            ...
            #expect(WCAG.contrast(ink, bg) >= Self.aa)
        }
    }
    ...
    @Test func subtleInkDoesNotRegressInDarkMode() {
        for (light, dark) in [
            ...
            let lightRatio = WCAG.contrast(Theme.Hex.inkSub.light, light)
            let darkRatio = WCAG.contrast(Theme.Hex.inkSub.dark, dark)
    ...
    @Test func whiteTextOnFilledChipsIsReadableInDarkMode() {
        for pair in [Theme.Hex.fillStrong, Theme.Hex.fillMuted] {
            #expect(WCAG.contrast(Self.white, pair.dark) >= Self.aa)
        }
        // 控えめなチップはライトの 3.32:1 より良くなる（9.05:1）。
        #expect(WCAG.contrast(Self.white, Theme.Hex.fillMuted.dark)
                >= WCAG.contrast(Self.white, Theme.Hex.fillMuted.light))
    }
    ...
    @Test func accentsStayReadableOnDarkSurfaces() {
        for accent in Theme.Hex.accents {
            let onDarkBackground = WCAG.contrast(accent, Theme.Hex.background.dark)
            let onDarkSurface = WCAG.contrast(accent, Theme.Hex.surface.dark)
            #expect(onDarkBackground >= WCAG.contrast(accent, Theme.Hex.background.light))
            #expect(onDarkSurface >= WCAG.contrast(accent, Theme.Hex.surface.light))
    ...
    @Test func textOnAccentFillsMeetsAA() {
        for fill in Theme.Hex.Fill.all {
            #expect(WCAG.contrast(Theme.Hex.onAccent, fill) >= Self.aa)
        }
    }
    ...
    @Test func whiteTextOnAccentFillsWouldFailAA() {
        for fill in Theme.Hex.Fill.all {
            #expect(WCAG.contrast(Self.white, fill) < Self.aa)
        }
    }
    ...
    @Test func onlyCoralAndPurpleDifferBetweenTextAndFill() {
        ...
        // 寄ってしまい、ダークの地の上で沈む）。
        #expect(WCAG.relativeLuminance(Theme.Hex.Fill.coral) > WCAG.relativeLuminance(Theme.Hex.coral))
        #expect(WCAG.relativeLuminance(Theme.Hex.Fill.purple) > WCAG.relativeLuminance(Theme.Hex.purple))
    }
    ...
    @Test func accentFillsAreNotUsableAsTextOnLightBackgrounds() {
        for fill in [Theme.Hex.Fill.coral, Theme.Hex.Fill.purple] {
            #expect(WCAG.contrast(fill, Theme.Hex.background.light) < Self.aa)
        }
    }
    ...
    @Test func cardRequiresABorderToSeparateFromBackground() {
        #expect(WCAG.contrast(Theme.Hex.surface.dark, Theme.Hex.background.dark) < 1.5)

...
/// CPU 同士の対局の計測（#1401）。**CI と通常の `swift test` では走らせない**
/// （環境変数 `CPU_BENCH=1` を付けたときだけ動く）。
///
/// 実行例: `CPU_BENCH=1 swift test --package-path Packages/GameKit --filter CPUBenchTests`
/// （デバッグビルドでは遅いので、対局は `swiftc -O -D OTHELLO_BENCH_STANDALONE` の単体バイナリでも回せる）
enum CPUBench {
...
struct CPUBenchTests {
    ...
    /// （会長決裁 2026-09-26。引き分けは負けに数えない）。`CPU_BENCH_PAIRS`（既定 20）組 × 先後。

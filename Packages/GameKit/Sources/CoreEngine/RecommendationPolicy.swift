...
public enum RecommendationPolicy {
    ...
    public static let candidateTable: [String: [String]] = [
        ...
        // 第1候補は本来ブロックならべ（#493）だったが、v1.1.4のハブから次版へ持ち越したため
        // （#642）、同じ1人用数字パズルの数独に差し替えた。
        "2048":          ["sudoku", "minesweeper", "blocks"],
        // ブロック崩し（#463・アクション枠の1本目）。同じ1人用スコアアタックの 2048 が最も近く、
        // 次いで1人で盤面を消していくマインスイーパー、手軽に終わる神経衰弱の順。
        // 第1候補をチャリンコおじさん（#494）にした。もう1本のアクション枠で、
        // 「反射で遊ぶ」手触りがいちばん近い。
        "blocks":        ["runner", "2048", "minesweeper"],
        // 第3候補を 15パズル（#1314）に差し替えた。同じ「1人で盤面を詰める」数字パズルで、麻雀ソリティアより近い
        // （麻雀ソリティアは麻雀・数独・スパイダーから引き続き出る）。
        ...
        // 次いで役の強さを競うポーカー、札を配って遊ぶブラックジャック。#642 で v1.1.4 から
        // 持ち越したぶんを #668 で戻した。
        "hanafuda":      ["daifugo", "poker", "blackjack"],
        // カードしりとり（#1243）。同じ絵札を使う神経衰弱が最も近く（絵柄も共有する・#1244）、
        // 次いで「CPU と札を取り合う」花札こいこい、手番制で札を出し合う大富豪。
        "shiritori":     ["concentration", "hanafuda", "daifugo"],
        // 15パズル（#1314）。1人で盤面を詰める同じ数字パズルの数独・2048 が近く、
        // 手軽に終わるマインスイーパーを第3候補に置く。
        "fifteen":       ["sudoku", "2048", "minesweeper"],
        // ブロックならべ（#493）のキーは無い。#642 で v1.1.4 のハブから外したまま
        // #603 の差し替え判断が続いているため、レコメンドの起点にもならない。他エントリの
        // 値からも除いてある（`AppEnvironment.registry` に戻すときはここも復元すること）。
    ]

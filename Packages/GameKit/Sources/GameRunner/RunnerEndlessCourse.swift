...
public enum RunnerEndlessCourse {
    ...
    ///
    /// イノシシ（`i`）の重みは #1172（会長 QA「エンドレスでイノシシの頻度が低い」）で 2 → 4 に
    /// 上げた。解禁が全部品でいちばん遅い（8,192）上に、岩の手前で止まると動かない置物になる
    /// （#801・意図的な仕様）ため、他の部品と同じ重みのままでは実際に突進してくる場面が少なく
    /// 感じられていた。解禁距離そのものは変えていない。
    ///

...
public struct RunnerField: Equatable, Sendable {
    ...
    public enum Metrics {
        ...
        /// 画面に見える縦幅。
        ///
        /// 元は 80（`width` に対してほぼ正方形）だったが、「ゲーム画面が正方形である意味がない」
        /// という会長QAを受けて縦長に変更（2026-09-10）。`width` はそのまま
        /// （横方向に見える地形の量・1 単位の大きさは変えたくない）、**縦だけ**伸ばして
        /// `RunnerView` 側でコースを画面の下まで大きく広げられるようにしてある。
        /// 増えた縦の余白は空だけにせず、`RunnerScene` の丘レイヤー・雲の配置で埋める。
        public static let height: Double = 140
        /// 地面の高さ（走者の足がここに乗る）。
        ///

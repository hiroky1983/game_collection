...
public enum RunnerRules {
    ...
    /// このフレームは「計時の穴」とみなす、という `dt` の下限（秒・#1386）。
    ///
    /// 一時停止・リザルト中は描画ループごと止める一方で時計は進み続けるため、再開の 1 フレーム目には
    /// 止めていた時間がまるごと `dt` として渡る。上の `maxStep` で刻んでも**そのフレームだけ 3 倍速で
    /// 進む**ので、この値を超えたフレームは進めずに時計だけ合わせ直す（ブロック崩しの
    /// `BlocksRules.staleFrameThreshold` と同じ値）。60fps の 15 フレームぶんで、通常のフレーム落ちでは届かない。

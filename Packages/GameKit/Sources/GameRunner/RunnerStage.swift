...
public enum RunnerRules {
    ...
    /// 上回っていれば、ここまで切り詰める。**タップしてすぐ離す（≒ 0 秒で離す）とほぼ
    /// この初速からの弾道になり**、頂点は `jumpCutVelocity² / (2·gravity)` ≒ 2.25 と
    /// 最も低い障害物（`RunnerHazardKind.lowBlock` = 5）にも届かないごく小さなホップになる。
    /// 離すタイミングが遅くなる（≒ 長く押し続ける）ほど、離した瞬間の `vy` はすでに
    /// 重力で減っているためこの切り詰めの影響が薄れ、十分長く押せば（目安: 自然に `vy` が
    /// この値を下回るまで、押してから (`jumpVelocity` − `jumpCutVelocity`) / `gravity` ≒
    /// 0.225 秒）切り詰め無しの全弾道（頂点 `jumpApex` ≒ 14.06）まで伸びる。

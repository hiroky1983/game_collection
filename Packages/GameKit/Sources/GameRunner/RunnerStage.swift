...
public enum RunnerRules {
    ...
    /// 高さ。同時に、階段状に段を重ねた場合でも 1 段ぶん（8）ずつなら常に 1 回のジャンプで
    /// 上がれる——2 段ぶん（16）は頂点を超えるので、段を飛ばして登ることはできない。
...
public extension RunnerStage {
    ...
    /// - 長さも 1 区画ずつ伸びる（1 面 12 区画 → 18 面 29 区画。おおよそ 23 秒 → 27 秒）
    ...
    ///
    /// `P`（乗れる台座・#674）はステージ 16〜18 にだけ置いてある。台座は障害ではないので
    /// 上の「障害の割合」には数えない——代わりに**前後 1 区画ずつの平地を必ず連れて行く**ので、
    /// 1 基あたり実質 3 区画以上を使う。既存 15 ステージのパターン文字列は 1 文字も変えていない
    ...
    private static let patterns: [String] = [
        ...
        // ここから #674 の「乗れる台座」枠。**台座の前後の区画は必ず平地（`-`）にする**。
        ...
        // が機械的に確かめる）。台座の上には障害・アイテムを置かない（第1弾の決裁どおり）。
        "--t2-n-PP-t-3t2-PPP-n-t3t--",   // 16: 27区画・障害11個＋台座2基。台座に乗る・降りるを覚える
        "--3t-n-PP-PP-2-t3-PPP-t-n2--",  // 17: 28区画・障害9個＋台座3基。台座を乗り継ぐ（連続台座）
        "--3t2-b-PP-t3-b-PPP-t2-b-n3--", // 18: 29区画・障害12個（岩穴9+鳥3）＋台座2基。台座と鳥・岩の複合
    ]
}

...
public enum RunnerRules {
    ...
    ///
    /// 上限まで約 1.5 秒。障害が詰まった後半のステージでは跳ぶたびに巻き戻されるので、
    /// 上限まで乗るのは平地が続く区間だけになる。
    public static let pedalGain: Double = 0.60
    /// 空中にいるあいだに乗りが落ちる速さ（毎秒）。
    ///
    /// 最小のジャンプ（滞空 `jumpAirTime` = 0.75 秒）でおよそ 0.45 落ちる = 乗りが 1 度で
    /// ほぼ振り出しに戻る。押し続けて高く跳べば滞空が伸び、着地後の速さがさらに落ちる。

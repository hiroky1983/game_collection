...
public enum SolitaireDealer {
    ...
    ///
    /// - Parameter rules: 局に焼き込むルール（#498）。**配札そのものは種だけで決まる**ので、
    ///   同じ種ならめくり枚数を変えても並びは1枚も動かない。
    public static func deal(seed: UInt64, rules: SolitaireRuleSet = .standard) -> SolitaireBoard {
        ...
        return SolitaireBoard(
            tableau: tableau,
            stock: Array(deck[index...].reversed()),
            rules: rules
        )
    }

    /// 出題に使う、ソルバーで勝ち筋を確認済みの種（1枚めくり）。
    ///
    ...
    /// 出題に使う、ソルバーで勝ち筋を確認済みの種（**3枚めくり専用**・#498）。
    ///
    /// **1枚めくりで解ける配札が3枚めくりでも解けるとは限らない**（山札の3枚に2枚は
    /// 一番上に来ないため、使える札が減る）。モードごとに別の検証を通した配列を持つのが要で、
    /// 片方を流用すると「クリアできると謳った配札が実は詰んでいる」ことが起きる。
    public static let verifiedSeedsDraw3: [UInt64] = solitaireVerifiedSeedsDraw3

    /// そのモードで検証済みの種の一覧。
    public static func verifiedSeeds(for mode: SolitaireDrawMode) -> [UInt64] {
        mode == .three ? verifiedSeedsDraw3 : verifiedSeeds
    }

    /// 出題用に1つ選ぶ。
    public static func randomVerifiedSeed<G: RandomNumberGenerator>(
        for mode: SolitaireDrawMode = .one,
        using rng: inout G
    ) -> UInt64 {
        let seeds = verifiedSeeds(for: mode)
...
public typealias SolitaireSeededGenerator = SplitMix64

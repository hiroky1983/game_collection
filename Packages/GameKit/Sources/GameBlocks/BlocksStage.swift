...
public extension BlocksStage {
    /// 球の速さの下限（ステージ 1）。
    ///
    /// **盤の高さに合わせて決める**。#597 で盤を 150 → 100 に縮めたため、ブロックの最下段
    /// （7 段のステージで y = 59）から発射位置（y = 13.3）までの可動域が 87.7 → 45.7 単位に
    /// 縮んだ。速さを 70 のまま据え置くと落球までの猶予が 1.25 秒 → 0.65 秒に半減し、
    /// 盤を広げただけのはずが難度が上がる。45 に下げて猶予を 1.02 秒まで戻した。
    /// これ以上下げると横 100 単位を渡るのに 2.5 秒以上かかって間延びするので、
    /// 元の 1.25 秒までは戻していない。
    static let baseSpeed: Double = 45
    /// 1 ステージ進むごとに増える速さ。最終ステージ（12）で 73.6 = 初速の約 1.6 倍になる
    /// （上の変更前と同じ比。難度の伸び方は変えていない）。
    static let speedStep: Double = 2.6

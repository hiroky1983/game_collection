...
public enum RecommendationPolicy {
    ...
    public static let candidateTable: [String: [String]] = [
        ...
        // 第1候補は本来ブロックならべ（#493）だったが、main では v1.1.4 開発中のため
        // 未登録（#629）。同じ1人用数字パズルの数独に差し替えた。
        "2048":          ["sudoku", "minesweeper", "blocks"],
        // ブロック崩し（#463・アクション枠の1本目）。同じ1人用スコアアタックの 2048 が最も近く、
        // 次いで1人で盤面を消していくマインスイーパー、手軽に終わる神経衰弱の順。
        // 第1候補は本来チャリンコおじさん（#494）だったが、main では v1.1.4 開発中のため
        // 未登録（#629）。手軽に終わる神経衰弱に差し替えた。
        ...
        // 第3候補は本来花札こいこい（#495）だったが、main では v1.1.4 開発中のため
        // 未登録（#629）。同じトランプの神経衰弱に差し替えた。
        ...
        // 第1候補は本来フリーセル（#492）だったが、main では v1.1.4 開発中のため
        // 未登録（#629）。同じトランプの大富豪に差し替えた。
        "solitaire":     ["daifugo", "mahjong", "concentration"],
        // freecell・blockpuzzle・runner・hanafudaのキーは無い。v1.1.4開発中でmainには
        // まだ登録していないため（#629。main = App Store公開済みの集合という規約に対し、
        // main直マージで混入していた4ゲームを差し戻した）、レコメンドの起点にもならない。
        // 他エントリの値からも除いてある。v1.1.4公開時のrelease/v1.1.4→mainの取り込みで
        // registryに戻すときは、ここも復元すること。
    ]

...
public struct HanafudaRuleSheet: View {
    ...
    public var body: some View {
        // 他ゲームの「くわしいルール」（`FreeCellRuleSheet` 等）はすべて `ScrollView` で包んでいるのに
        // ここだけ生の `VStack` だった。3 セクション・計12行が1画面に収まらない端末やダイナミック
        // タイプ拡大時に、下側が切れて触れなくなる（会長QA「デザインが崩れてる」で発覚）。
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                // 札の絵柄だけでは光札とタネ札を見分けられないため、上端の帯で month と kind を
                // 出している（#602）。記号を足した以上、読み方を引ける場所も要る。
                section("札の見かた", [
                    "札の上の帯に、左が月・右が種別（光・タネ・短・カス）を出しています。",
                    "合わせられるのは月が同じ札どうしです。",
                    "役は種別ごとの枚数で決まります。帯の色も種別に対応しています。",
                    "短冊の帯は、赤短が赤・青短が青です。",
                ])
                section("進行", [

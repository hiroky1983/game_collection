...
// 駒の意匠「リアル」（#1015）。駒を回転体として扱い、断面を細い帯に刻んで帯ごとに円筒の
// グラデーションを敷く。輪郭線は引かず、面の明暗と接地影だけで形を読ませる（決裁 2026-09-21）。
// 形・素材はすべて自作で、フォント・他者の画像・3D 素材は使わない（#462）。

// MARK: - A′: 2D の限界（回転体を「行ごとの円筒シェーディング」で描く）

/// 光源は左上手前（既存の碁石・立体駒と同じ向き）。
private struct RealV3 { var x, y, z: Double
    var len: Double { (x*x + y*y + z*z).squareRoot() }
    var unit: RealV3 { let l = len; return RealV3(x: x/l, y: y/l, z: z/l) }
    func dot(_ o: RealV3) -> Double { x*o.x + y*o.y + z*o.z }
}
private struct RealRGB { var r, g, b: Double
    func mixed(_ o: RealRGB, _ t: Double) -> RealRGB { RealRGB(r: r+(o.r-r)*t, g: g+(o.g-g)*t, b: b+(o.b-b)*t) }
    var color: Color { Color(.sRGB, red: min(1, max(0, r)), green: min(1, max(0, g)), blue: min(1, max(0, b))) }
}

private let lightDir = RealV3(x: -0.55, y: 0.65, z: 0.55).unit
private let halfVec = RealV3(x: lightDir.x, y: lightDir.y, z: lightDir.z + 1).unit

private struct ChessRealMaterial {
    let albedo: RealRGB
    let shininess: Double
    let specular: Double
    let envAmount: Double
    static func of(_ c: ChessColor) -> ChessRealMaterial {
        c == .white
            ? ChessRealMaterial(albedo: RealRGB(r: 0.97, g: 0.93, b: 0.83), shininess: 38, specular: 0.30, envAmount: 0.22)
            : ChessRealMaterial(albedo: RealRGB(r: 0.19, g: 0.14, b: 0.10), shininess: 70, specular: 0.95, envAmount: 0.60)
    }
}

/// 法線 n の点の色。拡散＋ブリン鏡面＋周縁の写り込み（空の白と盤の照り返しの茶）。
private func shade(_ n: RealV3, _ m: ChessRealMaterial) -> RealRGB {
    let hemi = n.y * 0.5 + 0.5
    let sky = RealRGB(r: 0.92, g: 0.93, b: 0.96)
    let bounce = RealRGB(r: 0.50, g: 0.36, b: 0.22)
    let ambient = bounce.mixed(sky, hemi)
    let ndl = max(0, n.dot(lightDir))
    var c = RealRGB(r: m.albedo.r * (ambient.r * 0.42 + ndl * 0.82),
    ...
    let env = bounce.mixed(RealRGB(r: 1, g: 1, b: 1), hemi)
    c = RealRGB(r: c.r + spec + env.r * fres * m.envAmount,
            g: c.g + spec + env.g * fres * m.envAmount,
            b: c.b + spec + env.b * fres * m.envAmount)
    return c
}

private let realGroundY: CGFloat = 0.93   // 接地線（セル上端からの比）
private let realScale: CGFloat = 0.90     // 高さ 1.0 → セル高の 0.90

struct ChessRealPieceCanvas: View {
    let piece: ChessPiece
    let size: CGFloat

    var body: some View {
        let mat = ChessRealMaterial.of(piece.color)
        return Canvas { ctx, sz in
            let W = sz.width, H = sz.height
            func pt(_ dx: CGFloat, _ y: CGFloat) -> CGPoint {
                CGPoint(x: W * (0.5 + dx * realScale), y: H * (realGroundY - y * realScale))
            }
            // 接地影
            let shadowRect = CGRect(x: W * 0.10, y: H * (realGroundY - 0.045), width: W * 0.80, height: H * 0.10)
            ctx.drawLayer { l in
    ...
    private static func drawRevolved(_ ctx: inout GraphicsContext, type: ChessPieceType, W: CGFloat, H: CGFloat,
                                     mat: ChessRealMaterial, pt: (CGFloat, CGFloat) -> CGPoint) {
        ...
        for i in 0..<(prof.count - 1) {
            ...
            for k in 0...steps {
                ...
                let n = RealV3(x: nx * sin(th), y: ny, z: max(0.0001, nx * cos(th)))
    ...
    private static func cylinder(_ ctx: inout GraphicsContext, rect: CGRect, radius: CGFloat, mat: ChessRealMaterial,
                                 centerX: CGFloat, W: CGFloat) {
        var stops: [Gradient.Stop] = []
        let steps = 16
        for k in 0...steps {
            let th = -Double.pi / 2 + Double.pi * Double(k) / Double(steps)
            stops.append(.init(color: shade(RealV3(x: sin(th), y: 0, z: cos(th)).unit, mat).color,
    ...
    private static func drawRookCrown(_ ctx: inout GraphicsContext, W: CGFloat, H: CGFloat, mat: ChessRealMaterial,
        ...
        // 銃眼（奥の内壁が見える）
        let inner = shade(RealV3(x: 0.3, y: 0.2, z: 1).unit, ChessRealMaterial(albedo: mat.albedo.mixed(RealRGB(r: 0, g: 0, b: 0), 0.5),
    ...
    private static func sphere(_ ctx: inout GraphicsContext, c: CGPoint, r: CGFloat, mat: ChessRealMaterial) {
        var stops: [Gradient.Stop] = []
        let steps = 12
        ...
        for k in 0...steps {
            ...
            stops.append(.init(color: shade(RealV3(x: nx, y: ny, z: nz).unit, mat).color, location: t))
        }
    ...
    private static func drawQueenBalls(_ ctx: inout GraphicsContext, W: CGFloat, H: CGFloat, mat: ChessRealMaterial,
                                       pt: (CGFloat, CGFloat) -> CGPoint) {
    ...
    private static func drawKingCross(_ ctx: inout GraphicsContext, W: CGFloat, H: CGFloat, mat: ChessRealMaterial,
                                      pt: (CGFloat, CGFloat) -> CGPoint) {
        ...
        for k in 0...12 {
            ...
            stops.append(.init(color: shade(RealV3(x: 0, y: -sin(th), z: cos(th)).unit, mat).color, location: Double(k) / 12))
        }
    ...
    private static func drawKnight(_ ctx: inout GraphicsContext, W: CGFloat, H: CGFloat, mat: ChessRealMaterial,
                                   pt: (CGFloat, CGFloat) -> CGPoint) {
        // 台座（回転体）
        ...
        var body = Path()
        unit.forEach { el in
            func m(_ p: CGPoint) -> CGPoint { let (dx, y) = ChessRealProfiles.knightMap(p.x, p.y); return pt(dx, y) }
            switch el {
        ...
        for k in 0...16 {
            ...
            stops.append(.init(color: shade(RealV3(x: sin(th), y: 0.15, z: cos(th)).unit, mat).color, location: t))
        }

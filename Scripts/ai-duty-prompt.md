...
自分が PR を作成したら、**直後に `gh pr merge <番号> --merge --auto` で auto-merge を仕込む**こと。CI 通過とスレッド解決が揃い次第 GitHub が自動でマージし、「完成したのに誰もマージしない」状態を構造的に防ぐ。レビュー指摘が来たら通常どおり消化すれば、解決時点で自動マージされる。
...
3. 状態に応じて回収する:
   - **PR まで出来ている** → `ai:in-progress` は残したまま、PR 側を通常のレビュー・マージフロー（セクション1）で前に進める。孤児ではないので回収不要。
